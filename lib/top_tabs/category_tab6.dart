...
class CategoryTab6 extends StatefulWidget {
  ...
  CategoryTab6({Key? key, required this.categoryId, required this.scaffoldKey})
      : super(key: key);

  @override
  _CategoryTab6State createState() => _CategoryTab6State();
}

class _CategoryTab6State extends State<CategoryTab6> {
  @override
  void initState() {
    super.initState();
    if (this.mounted) {
      if (context.read<CategoryTab6Bloc>().articles.isEmpty) {
        context.read<CategoryTab6Bloc>().fetchData(widget.categoryId, mounted);
      }
    }
  }
