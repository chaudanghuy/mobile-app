...
class CategoryTab5 extends StatefulWidget {
  ...
  CategoryTab5({Key? key, required this.categoryId, required this.scaffoldKey})
      : super(key: key);
...
class _CategoryTab5State extends State<CategoryTab5> {
  ...
  void initState() {
    super.initState();
    if (this.mounted) {
      if (context.read<CategoryTab5Bloc>().articles.isEmpty) {
        context.read<CategoryTab5Bloc>().fetchData(widget.categoryId, mounted);
      }
    }
  }
